...
#---My Solution---
=begin
  The string will not be modified outside of the method.  a_string_param is being re-assigned inside the method
    so the modified string will have a different object ID than my_string outside of the method.
  The array will be modified because the shovel << operator adds the argument to the end of the array.  This method will
    change the string outside of the method definition.
...
The string argument is passed to the method as a reference to an object of type String. Similarly, 
...
the method initializes a new local variable for both the string and the array and assigns each reference to the new local variables. 
